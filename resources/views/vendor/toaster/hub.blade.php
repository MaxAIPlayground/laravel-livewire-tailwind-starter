<div role="status" id="toaster" x-data="toasterHub(@js($toasts), @js($config))" @class([
    'fixed z-50 p-4 w-full flex flex-col pointer-events-none sm:p-6 rounded-sm',
    ...
    <template x-for="toast in toasts" :key="toast.id">
        ...
        >
            <div x-text="toast.message"
               class="inline-block select-none px-6 py-3 text-sm w-full rounded-md shadow-sm {{ $alignment->is('bottom') ? 'mt-3' : 'mb-3' }}"
               :class="toast.select({ error: 'bg-error', info: 'bg-base-100', success: 'bg-success', warning: 'bg-warning' })"
            ></div>
